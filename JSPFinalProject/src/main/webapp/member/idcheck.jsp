...
$(function(){
	...
	$('#checkBtn'.click(function(){
		...
		$.ajax({
			...
			data:{"id":id},
			success:function(result){	// 실행결과를 가져오는 result => callback 함수
				alert("result: "+result)
			...
			<table class="table">
				<tr>
					<td>ID: <input type="text" name="id" id=id size=15
						class="input-sm"> <input type="button" value="중복체크"
						class="btn btn-sm btn-warning" id=checkBtn>
					</td>
				</tr>
				<tr>
				...
				<tr id="ok">
					<td class="text-center"><input type="button" value="확인"
						id="okBtn" class="btn btn-sm btn-primary"></td>
				</tr>
			</table>
		</div>
	</div>
