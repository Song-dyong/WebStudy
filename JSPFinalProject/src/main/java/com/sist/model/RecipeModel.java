...
public class RecipeModel {
	...
	public String chef_list(HttpServletRequest request, HttpServletResponse response) {
		...
		// post 방식은 인코딩을 해야하지만, uri에 담겨서 넘어오는 get 방식은 한글이 깨지지 않는다. 윈도우 10 이상만... 
...
//		System.out.println(chef);
		...
		int totalpage=0;
		if(sel.equals("all")) {
			list=dao.chefRecipeListData(curpage, chef);
			totalpage=dao.chefListTotalPage(chef);
		}
		else {
			list=dao.chefFindRecipeListData(curpage, chef, fd);
