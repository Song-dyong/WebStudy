...
/*
		JDBC => 원시 소스: 전부 코딩 
		DBCP (속도의 최적화)				=>1차 프로젝트
		ORM: MyBatis (2차) / JPA (3차) / Hibernate
		
		실무에서 오라클 사용 x => MySQL, MariaDB
...
public class StudentList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 전송 타입: html (문자는 utf-8)
		response.setContentType("text/html;charset=UTF-8");
		// 문자 출력하는 역할 out.println()
		...
		for(StudentVO vo:list)
		{
			...
			out.println("<a href=StudentUpdate?hakbun="+vo.getHakbun()+" class=\"btn btn-xs btn-primary\">수정</a>&nbsp;");
			out.println("<a href=StudentDelete?hakbun="+vo.getHakbun()+" class=\"btn btn-xs btn-info\">삭제</a>");
			out.println("</td>");
			out.println("</tr>");
		}
		out.println("</table>");
