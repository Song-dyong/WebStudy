...
public class FoodDAO {
	...
	public FoodBean foodDetailData(int fno) {
		...
		try {
			...
			*/
			String sql="SELECT fno, poster, name, tel, score, address, type, price, time, parking, menu, hit "
					+ "FROM food_location "
					+ "WHERE fno=?";
			ps=conn.prepareStatement(sql);
			ps.setInt(1, fno);
			ResultSet rs= ps.executeQuery();
			rs.next();	// 데이터가 있는 메모리에 커서를 위치시킨다.
			vo.setFno(rs.getInt(1));
			...
			rs.close();
			// BLOB, BFILE : rs.getInputStream()으로 읽어야 함 / 날짜 : rs.getDate()
		} catch (Exception e) {
			e.printStackTrace();
		}
		finally {
