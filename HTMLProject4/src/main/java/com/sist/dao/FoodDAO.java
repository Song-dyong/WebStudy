...
public class FoodDAO {
	...
	public List<CategoryVO> food_category_list() {
		...
		try {
			...
			// String sql="SELECT /*+ INDEX_ASC(food_category fc_cno_pk)*/
			// cno,title,subject,poster FROM food_category";
...
//						+"WHERE address LIKE '%'||?||'%'";
			...
			String sql = "SELECT fno,name,poster,score,num " + "FROM (SELECT fno,name,poster,score,rownum as num "
					+ "FROM (SELECT fno,name,poster,score " + "FROM food_location "
					+ "WHERE address LIKE '%'||?||'%')) " + "WHERE num BETWEEN ? AND ?";
			ps = conn.prepareStatement(sql);
			int rowSize = 12;
